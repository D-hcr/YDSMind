...
export default function QuestionDetailModal({ question, onClose }: QuestionDetailModalProps) {
  ...
  const expl = explanationOf(question);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4">
      <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl border border-slate-700 bg-slate-950 p-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <p className="text-sm uppercase tracking-[0.2em] text-cyan-300">
              {question.examType ?? '—'} / {question.topicId ?? question.categoryId} • {question.module ?? question.source}
            </p>
        ...
        <div className="mb-8">
          <h2 className="text-xl font-semibold text-white leading-relaxed mb-6">{stem}</h2>
            ...
            {(['A', 'B', 'C', 'D', 'E'] as const).map((letter) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => {
                    if (!showAnswer) setUserAnswer(letter);
                  }}
                  disabled={showAnswer}
        ...
        {showAnswer && userAnswer && (
          ...
          >
            <p className={`font-semibold mb-2 ${isCorrect ? 'text-emerald-300' : 'text-red-300'}`}>
            ...
            <p className="text-sm text-slate-400">
              <span className="font-semibold text-slate-300">Açıklama:</span> {expl}
            </p>
          </div>
        )}
      </div>
